...
export class Cl_vLaboratorio {
    ...
    public async cargarOrdenesFinalizadas(): Promise<void> {
        ...
        finalizados.forEach(ex => {
            ...
            const nombresEstudios = ex.estudiosSolicitados && ex.estudiosSolicitados.length > 0
                ? ex.estudiosSolicitados.map(est => est.nombre).join(', ')
                : 'Ninguno';

            // CONSTRUCCIÓN SEGURA DEL MENSAJE PARA EVITAR CAÍDAS DE SCRIPT
            let listadoEstudiosText = '';
            try {
                if (ex.estudiosSolicitados) {
                    listadoEstudiosText = ex.estudiosSolicitados.map(estudio => {
                        const resVal = ex.resultados && estudio.id ? ex.resultados[estudio.id] : undefined;
                        const resultado = resVal || 'Pendiente';
                        return `▪️ *${estudio.nombre}:*\n   Resultado: *${resultado}*\n   Valores Ref: ${estudio.valorReferencia || 'N/A'}\n\n`;
                    }).join('');
                }
            } catch (err) {
                listadoEstudiosText = '▪️ Ver detalles en sistema.\n\n';
            }
            ...
            // DISEÑO REDISEÑADO: Estructura limpia donde el botón es un bloque independiente que no se estira
            tarjeta.innerHTML = `
            ...
            // Validación por si no tiene teléfono registrado
            const botonEnlace = tarjeta.querySelector('.enlace-whatsapp-directo') as HTMLAnchorElement;
            botonEnlace.addEventListener('click', (e) => {
                if (!ex.celular || ex.celular.trim() === '') {
                    e.preventDefault();
