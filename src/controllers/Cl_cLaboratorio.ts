...
export class Cl_cLaboratorio {
    private servicio: Cl_sLaboratorio;

    constructor() {
        this.servicio = new Cl_sLaboratorio();
    }

    public async obtenerTodos(): Promise<Cl_mEstudio[]> {
        const datos = await this.servicio.consultarEstudios();
        const listaEstudios = datos.map((e: any) => new Cl_mEstudio(e.nombre, Number(e.precio), e.valorReferencia, e.id));
        const labModel = new Cl_mLaboratorio(listaEstudios);
        return labModel.estudios;
    }

    public async registrarNuevoEstudio(nombre: string, precio: number, valorReferencia: string): Promise<void> {
        const payload = {
            nombre: nombre,
            precio: precio,
